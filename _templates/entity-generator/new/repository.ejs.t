...
async function insert<%= h.capitalize(name) %>(<%= name %>){
    ...
    try {
        const sql = "INSERT INTO <%= name %>s (nome, telefone) VALUES ($1, $2) RETURNING *"
...
async function update<%= h.capitalize(name) %>(<%= name %>){
    const conn = await connect()
    try {
        const sql= "UPDATE <%= name %>s SET NOME $1, TELEFONE $2 WHERE <%= name %>_id = $3 RETURNING"
        const values = [
            <%= name %>.nome,
            <%= name %>.telefone,
            <%= name %>.<%= name %>_id
...
async function get<%= h.capitalize(name) %>(<%= name %>){
    const conn = await connect()
    try {
       const res = await conn.query("SELECT * FROM <%= name %>s WHERE <%= name %>_id = $1", [
            <%= name %>_id,
        ])
        return res.rows[0]
    } catch (err) {
